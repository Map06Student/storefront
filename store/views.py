...
class OrderViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        # Get a serializer instance using CreateOrderSerializer
        serializer = CreateOrderSerializer(
            data=request.data, context={'user_id': self.request.user.id})
        # Validate the data
        serializer.is_valid(raise_exception=True)
        # Save the changes and get an order object
        order = serializer.save()
        # Get another serializer instance using OrderSerializer
        serializer = OrderSerializer(order)
        # Return a response with serializer.data as content
        return Response(serializer.data)
    ...
    def get_queryset(self):
        ...
        # Apply a filter and only retrieve orders for specific customer ID
        # First get only the customer ID from customer object
        customer_id = Customer.objects.only('id').get(user_id=user.id)
        # Finally return filtered queryset
